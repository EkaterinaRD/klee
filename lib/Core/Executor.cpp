...
cl::opt<bool>
    UseGEPOptimization("use-gep-expr", cl::init(true),
                       cl::desc("Make executor lazily initialize whole object "
                                "during accesses to its attributes."),
                       cl::cat(ExecCat));

cl::opt<ExecutionKind> ExecutionMode(
    "execution-mode",
    cl::values(
        clEnumValN(ExecutionKind::Forward, "forward",
                   "Use basic klee symbolic execution"),
        clEnumValN(ExecutionKind::Guided, "guided",
                   "Use basic klee symbolic execution with guided searcher"),
        clEnumValN(ExecutionKind::Bidirectional, "bidirectional",
                   "Use bidirectional execution") KLEE_LLVM_CL_VAL_END),
    cl::init(ExecutionKind::Bidirectional), cl::desc("Execution mode"),
    cl::cat(ExecCat));
...
llvm::cl::opt<unsigned> MaxFailedBranchings(
    ...
    llvm::cl::desc("start bidirectional execution after failing during some "
                   "branching this amount of times (default=1)."),
    llvm::cl::init(1), llvm::cl::cat(ExecCat));

cl::opt<std::string> SummaryDB("summary-db", cl::init(""),
                               cl::desc("Summary DB path"), cl::cat(ExecCat));

cl::opt<bool> ReplayStateFromProofObligation(
    "replay-state-from-pob", cl::init(false),
...
cl::opt<bool> UseSolverInComposition(
    "check-sat-compose", cl::init(false),
...
cl::opt<uint64_t> MaxRec("max-rec-depth",
...
Executor::setModule(std::vector<std::unique_ptr<llvm::Module>> &modules,
  ...
  if (StatsTracker::useStatistics() || userSearcherRequiresMD2U()) {
    statsTracker = std::make_unique<StatsTracker>(
...
void Executor::branch(ExecutionState &state,
  ...
  if (it != seedMap->end()) {
    ...
    for (std::vector<SeedInfo>::iterator siit = seeds.begin(),
      ...
      if (result[i]) {
        seedMap->pushBack(result[i], siit);
      }
    }
...
void Executor::executeGetValue(ExecutionState &state, ref<Expr> e,
  ...
  if (it == seedMap->end() || isa<ConstantExpr>(e)) {
    ...
    for (ExprHashSet::iterator vit = values.begin(), vie = values.end();
         vit != vie; ++vit)
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (!conflict.empty() && state.depth && !state.isIsolated()) {
        ...
        Transition failedTransition =
            std::make_pair(bi->getParent(), target->basicBlock);
      ...
      if (branches.first)
        successTransitions.insert(
            std::make_pair(bi->getParent(), bi->getSuccessor(0)));
      if (branches.second)
        successTransitions.insert(
            std::make_pair(bi->getParent(), bi->getSuccessor(1)));
  ...
  case Instruction::Switch: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
      ...
      for (ExprHashMap<BasicBlock *>::iterator it = expressionOrder.begin(),
  ...
  case Instruction::GetElementPtr: {
    ...
    if (UseGEPOptimization && !isa<ConstantExpr>(address) &&
...
void Executor::resolveExact(ExecutionState &state, ref<Expr> p,
                            ...
                            const std::string &name, KInstruction *target) {
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  ref<Expr> base = UseGEPOptimization && isGEPExpr(address)
                       ? gepExprBases[address].first
                       : address;
  unsigned size = UseGEPOptimization && isGEPExpr(address)
  ...
  incomplete = state.addressSpace.resolve(state, solver.get(), base, rl, true,
                                          false, 0, coreSolverTimeout);
  ...
  if (unbound) {
    if (incomplete) {
      ...
      ObjectPair p;
      bool wasBaseResolved =
          rl.size() == 1 && rl.begin()->first->getBaseExpr() == base;
      if (wasBaseResolved)
        needAddressLazyInitialization = true;
      else {
        p = lazyInitializeVariable(*unbound, base, false,
                                   target ? target->inst : nullptr, size);
      ...
      if (needAddressLazyInitialization) {
        if (!bound && unbound->isIsolated()) {
          p = lazyInitializeVariable(*unbound, address, false,
                                     target ? target->inst : nullptr, bytes);
        } else {
...
ObjectPair Executor::lazyInitialize(ExecutionState &state,
                                    const MemoryObject *mo,
                                    const Array *array) {
...
ObjectPair Executor::lazyInitializeVariable(ExecutionState &state,
  ...
  MemoryObject *mo =
      array->binding
          ? const_cast<MemoryObject *>(array->binding)
          : memory->allocate(size, isLocal, /*isGlobal=*/false, allocSite,
                             /*allocationAlignment=*/8, address);
  return lazyInitialize(state, mo, array);
}

ObjectPair Executor::transparentLazyInitializeVariable(
    ExecutionState &state, ref<Expr> address, const llvm::Value *allocSite,
    uint64_t size) {
  assert(!isa<ConstantExpr>(address));
  const Array *array = makeArray(state, size, "lazy_instantiation", address);
  MemoryObject *mo = array->binding
                         ? const_cast<MemoryObject *>(array->binding)
                         : memory->allocateTransparent(
...
const Array *Executor::makeArray(ExecutionState &state, const uint64_t size,
  ...
  const Array *array =
      arrayManager.CreateArray(uniqueName, size, isExternal, liSource);
...
const Array *Executor::makeArray(ExecutionState &state, const uint64_t size,
                                 const std::string &name, ref<Expr> liSource) {
  ...
  if (!replayKTest) {
    if (!array) {
      ...
      if (state.symbolicCounter < kleeSymbolic.size()) {
        ...
        array =
            makeArray(state, mo->size, name, true, mo->lazyInitializedSource);
      }
    }
...
void Executor::addAllocaDisequality(ExecutionState &state,
                                    const Value *allocSite, ref<Expr> address) {
  ...
  for (auto &symbolic : state.symbolics) {
    if (!symbolic.first->isLazyInitialized() && symbolic.first->size == size &&
      ...
      Instruction *inst =
          const_cast<Instruction *>(cast<Instruction>(allocSite));
      state.addConstraint(
          Expr::createIsZero(EqExpr::create(symbolicAlloca, address)),
          getKInst(inst));
    }
  }
}
...
void Executor::prepareSymbolicRegister(ExecutionState &state, StackFrame &frame,
                                       unsigned regNum) {
...
ref<Expr> Executor::makeSymbolicValue(Value *value, ExecutionState &state,
  ...
  MemoryObject *mo =
      array->binding
          ? const_cast<MemoryObject *>(array->binding)
          : memory->allocateTransparent(size, true, /*isGlobal=*/false, value,
...
bool Executor::getSymbolicSolution(const ExecutionState &state, TestCase &res) {
  solver->setTimeout(coreSolverTimeout);
...
TimingSolver *Executor::getSolver() { return solver.get(); }

time::Span Executor::getSolverTimeout() { return coreSolverTimeout; }
...
KBlock *Executor::getKBlock(llvm::BasicBlock &bb) {
  ...
  assert(F && this->kmodule->functionMap.find(F) !=
                  this->kmodule->functionMap.end());
  ...
  for (ExecutionState *reExState : statesDB) {
    ...
    while (reExecution) {
      ...
      if (reExState->transferToBB) {
        size_t indexPath = reExState->path.getCurrentIndex();
      ...
      if (reExState->node.path.size() == reExState->path.size()) {
        if (reExState->node.currPC == reExState->prevPC) {
...
bool Executor::checkSafeguardingData(ExecutionState *state) {
  bool i =
      (state->steppedInstructions == state->node.countInstrs) ? true : false;
  // bool h;
  bool b = (state->executionPath == state->node.executionPath) ? true : false;
...
void Executor::goForward(ref<BidirectionalAction> a) {
  ...
  ref<ForwardAction> action = cast<ForwardAction>(a);
...
void Executor::goBackward(ref<BackwardAction> action) {
  ...
  bool success =
      Composer::tryRebuild(*pob, *state, *newPob, conflictCore, rebuildMap);
  timers.invoke();

  if (success) {
    newPob->propagationCount[state]++;
    // goBackward assumes that the state and the proof obligation are
    // stack-compatible so we only need to pop the right amount of stack frames
    // from the proof obligation.
    if (state->stack.size() != 0) {
      for (auto it = state->stack.rbegin();
           it != std::prev(state->stack.rend()) && !newPob->stack.empty();
           it++) {
    ...
    if (state->isIsolated() && conflictCore.size()) {
      // summary->summarize(pob, makeConflict(*state, conflictCore),
      // rebuildMap);
      objectManager.summarize(pob, makeConflict(*state, conflictCore),
                              rebuildMap);
    }
  }
}
...
// bool klee::inReexecutionMode(ExecutionState *state) { return
// state->reExecuted; }