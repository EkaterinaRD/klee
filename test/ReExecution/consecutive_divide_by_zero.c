...
// RUN: %klee --output-dir=%t.klee-out1 -check-div-zero -emit-all-errors=0 --do-backward-first --write-to-db %t.bc 2> %t.log1
// RUN: %klee --output-dir=%t.klee-out2 -check-div-zero -emit-all-errors=0 --do-backward-first --write-to-db --summary-db=%t.klee-out1/summary.sqlite3 %t.bc 2> %t.log2
